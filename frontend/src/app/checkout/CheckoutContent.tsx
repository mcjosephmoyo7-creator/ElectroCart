  ...
  if (items.length === 0) {
    return (
      <div className="container-custom py-24 text-center">
        ...
        <h1 className="text-2xl font-bold text-slateText dark:text-white mb-2">Checkout</h1>
  ...
  return (
    <div className="container-custom py-8 lg:py-12">
      <nav className="flex items-center gap-1.5 text-sm text-muted mb-6" aria-label="Breadcrumb">
        ...
        <span className="opacity-50">/</span>
        <span className="text-slateText dark:text-white font-medium">Checkout</span>
      </nav>

      <h1 className="text-3xl lg:text-4xl font-bold text-slateText dark:text-white mb-8">Checkout</h1>
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${
                i === step
                  ...
                  : i < step
                    ...
                    : 'bg-white dark:bg-navy-200 text-muted border border-lineBorder dark:border-navy-50'
              }`}
            >
              {i < step ? <HiOutlineCheck className="w-4 h-4" /> : <span>{i + 1}</span>}
              {label}
            </div>
            {i < steps.length - 1 && <div className={`w-8 h-px ${i < step ? 'bg-success' : 'bg-lineBorder dark:bg-navy-100'}`} />}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
        <div className="bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl p-6 lg:p-8">
          {step === 0 && (
            <div>
              <h2 className="text-xl font-bold flex items-center gap-2.5 text-slateText dark:text-white mb-6">
          ...
          {step === 1 && (
            <div>
                  ...
                  <label
                    ...
                    className={`flex items-center gap-3 border rounded-xl px-4 py-3.5 cursor-pointer transition-all ${
                      form.paymentMethod === method.key
                        ...
                        : 'border-lineBorder dark:border-navy-100 hover:border-primary/40'
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    <span className="text-sm font-semibold text-slateText dark:text-white">{method.label}</span>
              ...
              {form.paymentMethod !== 'card' && (
                <div className="bg-body dark:bg-navy-100 border border-dashed border-lineBorder dark:border-navy-100 rounded-xl p-5 text-sm text-muted">
          ...
          {step === 2 && (
            <div>
              <h2 className="text-xl font-bold text-slateText dark:text-white mb-6">Review Your Order</h2>
                ...
                {items.map(({ product, quantity }) => (
                  <div key={product._id} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slateText dark:text-white line-clamp-1">{product.name}</p>
                      <p className="text-xs text-muted">Qty: {quantity}</p>
                    </div>
                    <span className="font-bold text-slateText dark:text-white">{formatPrice((product.discountPrice ?? product.price) * quantity)}</span>
                  </div>
                ))}

                <div className="grid grid-cols-2 gap-6 pt-4 border-t border-lineBorder dark:border-navy-100">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wide text-muted mb-2">Shipping To</p>
                    <p className="text-sm font-semibold text-slateText dark:text-white">{form.fullName}</p>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slateText dark:text-white capitalize">
          ...
          {/* Step nav */}
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-lineBorder dark:border-navy-100">
            <button
              onClick={goBack}
              disabled={step === 0}
              className="flex items-center gap-1.5 text-sm font-semibold text-muted hover:text-slateText dark:hover:text-white disabled:opacity-40 disabled:hover:text-muted transition-colors"
            >
        ...
        <aside className="h-fit lg:sticky lg:top-28">
          <div className="bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl p-6">
            <h3 className="font-bold text-slateText dark:text-white mb-4">Order Summary</h3>
            <div className="max-h-56 overflow-y-auto space-y-3 mb-4">
              {items.map(({ product, quantity }) => (
                <div key={product._id} className="flex items-center gap-3">
                  ...
                  <p className="flex-1 text-xs text-slateText dark:text-white line-clamp-1">{product.name}</p>
                  <span className="text-xs font-bold">{formatPrice((product.discountPrice ?? product.price) * quantity)}</span>
                </div>
              ))}
            </div>
            <div className="space-y-2 pt-4 border-t border-lineBorder dark:border-navy-100 text-sm">
              <div className="flex justify-between text-muted">
                <span>Subtotal</span><span className="text-slateText dark:text-white font-semibold">{formatPrice(subtotal)}</span>
              </div>
              <div className="flex justify-between text-muted">
                <span>Shipping</span>
                {shipping === 0 ? <span className="text-success font-semibold">FREE</span> : <span className="text-slateText dark:text-white font-semibold">{formatPrice(shipping)}</span>}
              </div>
              <div className="flex justify-between text-muted">
                <span>Tax</span><span className="text-slateText dark:text-white font-semibold">$0.00</span>
              </div>
            </div>
            <div className="flex justify-between items-center pt-4 border-t border-lineBorder dark:border-navy-100 mt-4">
              <span className="font-bold text-slateText dark:text-white">Total</span>
