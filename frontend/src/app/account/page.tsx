...
function AccountContent() {
  ...
  <h1 className="text-3xl lg:text-4xl font-bold text-slateText  mb-8">My Account</h1>
  ...
  <h2 className="font-bold text-slateText  truncate">{user?.username}</h2>
  ...
  <div className="flex items-center justify-between py-3 border-t border-lineBorder  text-sm">
  <span className="text-muted">Account type</span>
  <span className="font-semibold capitalize text-slateText ">{user?.role || 'customer'}</span>
  </div>
  <div className="flex items-center justify-between py-3 border-t border-lineBorder  text-sm">
  <span className="text-muted">Member</span>
  <span className="font-semibold text-slateText ">ElectroCart</span>
  </div>
  ...
  <h2 className="text-xl font-bold text-slateText  flex items-center gap-2.5">
  ...
  <h3 className="font-bold text-slateText  mb-1">No orders yet</h3>
  ...
  {orders.map((order) => (
  <div key={order._id} className="border border-lineBorder  rounded-xl p-4">
  <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
  <div>
  <p className="font-bold text-slateText  text-sm">
  ...
  <div className="h-px bg-lineBorder  mb-3" />

  <div className="flex items-center justify-between">
  <div className="flex -space-x-2">
  {(order.items || []).slice(0, 4).map((item, i) =>
  item.image ? (
  <div key={i} className="w-9 h-9 rounded-lg overflow-hidden border-2 border-white ">
  <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
  </div>
  ) : null
  )}
  {(order.items || []).length > 4 && (
  <div className="w-9 h-9 rounded-lg bg-body  border-2 border-white  flex items-center justify-center text-[10px] font-bold text-muted">
  +{order.items!.length - 4}
  </div>
  )}
  </div>
