...
export default function WishlistPage() {
  ...
  <h1 className="text-3xl lg:text-4xl font-bold text-slateText  flex items-center gap-3">
  ...
  <h2 className="text-xl font-bold text-slateText  mb-2">Your wishlist is empty</h2>
  ...
  return (
  <div key={product._id} className="group bg-white  border border-lineBorder  rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all">
  ...
  <h3 className="font-semibold text-slateText  line-clamp-1 text-sm hover:text-primary transition-colors">
  {product.name}
  </h3>
  </Link>
  ...
  className="flex-1 flex items-center justify-center gap-1.5 bg-navy  hover:bg-primary text-white py-2 rounded-lg text-xs font-semibold transition-colors"
  >
  ...
  className="w-9 h-8 border border-lineBorder  rounded-lg flex items-center justify-center text-muted hover:text-red-500 hover:border-red-300 transition-colors"
  ...
  {wishlistItems.length > 0 && (
  <div className="mt-16">
  <h2 className="text-2xl font-bold text-slateText  mb-6">Top Rated Picks</h2>
  <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
  {suggestions.map((p) => (
