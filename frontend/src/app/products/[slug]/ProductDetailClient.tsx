...
export default function ProductDetailClient({ product, images, specs, related }: ProductDetailClientProps) {
  ...
  <HiOutlineChevronRight className="w-4 h-4" />
  <span className="text-slateText  font-medium line-clamp-1">{product.name}</span>
  </nav>
  ...
  <div className="relative flex-1 aspect-square rounded-2xl overflow-hidden bg-body  border border-lineBorder ">
  <img
  ...
  <h1 className="mt-1.5 text-2xl lg:text-3xl font-bold text-slateText  leading-snug">
  {product.name}
  </h1>
  ...
  <span className="text-4xl font-bold text-slateText ">{formatPrice(displayPrice)}</span>
  {hasDiscount && (
  <>
  <span className="text-lg text-muted line-through mb-1">{formatPrice(product.price)}</span>
  <span className="mb-1.5 px-2 py-1 bg-accent/10 text-accent-dark  text-xs font-bold rounded-md">
  ...
  {product.stock <= 10 && (
  <span className="bg-accent/10 text-accent-dark  text-xs font-bold px-2 py-0.5 rounded">HURRY UP</span>
  )}
  </div>

  {/* Quantity + Add */}
  <div className="mt-6 flex flex-wrap items-center gap-4">
  <div className="flex items-center border border-lineBorder  rounded-xl overflow-hidden">
  <button
  onClick={() => setQuantity((q) => Math.max(1, q - 1))}
  className="w-11 h-12 flex items-center justify-center text-muted hover:text-primary hover:bg-body  transition-colors"
  aria-label="Decrease quantity"
  >
  <HiOutlineMinus className="w-4 h-4" />
  </button>
  <span className="w-12 text-center font-bold text-slateText ">{quantity}</span>
  <button
  ...
  className="w-12 h-12 border border-lineBorder  rounded-xl flex items-center justify-center text-muted hover:border-red-300 hover:text-red-500 transition-colors"
  ...
  <p className="font-semibold text-slateText ">Free Delivery</p>
  ...
  <div className="text-xs">
  <p className="font-semibold text-slateText ">Free Return</p>
  ...
  <div className="text-xs">
  <p className="font-semibold text-slateText ">Guarantee</p>
  ...
  {/* Tabs */}
  <div className="mt-14">
  <div className="flex gap-2 border-b border-lineBorder ">
  ...
  : 'border-transparent text-muted hover:text-slateText '
  }`}
  >
  ...
  <div className="max-w-3xl overflow-hidden rounded-xl border border-lineBorder ">
  {specs.map((spec, i) => (
  <div
  key={spec.key}
  className={`grid grid-cols-1 sm:grid-cols-3 px-5 py-3.5 text-sm ${
  i % 2 === 0 ? 'bg-white ' : 'bg-body '
  }`}
  >
  <span className="font-semibold text-slateText ">{spec.key}</span>
  ...
  <h2 className="text-2xl font-bold text-slateText ">You May Also Like</h2>
