...
export default function ShopContent({ initialSearch = '', initialCategory = '', initialBrand = '' }: ShopContentProps) {
  ...
  <div>
  <h3 className="font-bold text-slateText  mb-3 flex items-center gap-2">
  ...
  />
  <span className="text-sm text-slateText  group-hover:text-primary transition-colors">
  {cat.name}
  </span>
  ...
  <hr className="border-lineBorder " />
  ...
  : 'border-lineBorder  hover:border-accent/60 hover:shadow-card'
  }`}
  >
  <span className="transition-transform duration-300 group-hover:scale-110">
  <BrandLogo brand={brand} size={26} />
  </span>
  <span className="text-[11px] font-medium text-muted group-hover:text-accent  text-center leading-tight transition-colors">
  {brand.name}
  </span>
  </button>
  ))}
  ...
  <button onClick={clearAll} className="w-full text-center text-sm font-semibold text-red-500 hover:text-red-600 py-2 border border-lineBorder  rounded-lg hover:bg-red-50  transition-colors">
  Clear All Filters
  </button>
  )}
  </div>
  ...
  <div className="rounded-2xl border border-lineBorder  bg-white  p-12 text-center">
  <h3 className="text-xl font-bold text-slateText  mb-2">{title}</h3>
  ...
  <div className="sticky top-28 bg-white  border border-lineBorder  rounded-2xl p-6">
  <Sidebar />
  </div>
  </aside>
  ...
  className="w-full pl-11 pr-4 py-3 bg-white  border border-lineBorder  rounded-xl text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
  />
  {search && (
  ...
  className="lg:hidden flex items-center justify-center gap-2 bg-navy  text-white px-4 py-3 rounded-xl text-sm font-semibold"
  >
  ...
  className="bg-white  border border-lineBorder  rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-primary cursor-pointer"
  >
  {sortOptions.map((opt) => (
  ...
  {selectedCategories.length > 0 && (
  <span className="inline-flex items-center gap-1.5 bg-accent/10 text-accent-dark  text-xs font-semibold px-3 py-1.5 rounded-full">
  ...
  <span className="inline-flex items-center gap-1.5 bg-navy/10  text-navy  text-xs font-semibold px-3 py-1.5 rounded-full">
  ...
  Showing <span className="font-semibold text-slateText ">{Math.min(visibleCount, filtered.length)}</span> of{' '}
  <span className="font-semibold text-slateText ">{filtered.length}</span> products
  </p>
  {hasMore && (
  ...
  !loading && (
  <div className="text-center py-24 bg-white  border border-lineBorder  rounded-2xl">
  <p className="mb-4"><HiOutlineSearch className="w-16 h-16 inline-block text-muted/50" /></p>
  <h3 className="text-xl font-bold text-slateText  mb-2">No Product Available</h3>
  ...
  <div className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-white  overflow-y-auto p-6 animate-zoom-in">
  <div className="flex items-center justify-between mb-6">
  <h3 className="font-bold text-lg text-slateText ">Filters</h3>
  <button onClick={() => setMobileFiltersOpen(false)} className="p-2 text-muted hover:text-navy " aria-label="Close filters">
