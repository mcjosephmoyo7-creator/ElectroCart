  ...
  return (
    ...
    >
      ...
      <nav className="flex items-center gap-1.5 text-sm text-muted mb-6" aria-label="Breadcrumb">
        ...
        <span className="text-slateText dark:text-white font-medium">Cart</span>
      </nav>

      <h1 className="text-3xl lg:text-4xl font-bold text-slateText dark:text-white mb-2">Shopping Cart</h1>
      ...
      {items.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl">
          ...
          <h2 className="text-xl font-bold text-slateText dark:text-white mb-2">Your cart is empty</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
          {/* Items */}
          <div className="bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl overflow-hidden">
            <div className="hidden sm:grid grid-cols-[1fr_110px_110px_50px] gap-4 px-6 py-3 bg-body dark:bg-navy-100 text-xs font-bold text-muted uppercase tracking-wider border-b border-lineBorder dark:border-navy-100">
              <span>Product</span>
            ...
            {items.map(({ product, quantity }) => {
              ...
              return (
                <div key={product._id} className="grid grid-cols-[70px_1fr] sm:grid-cols-[1fr_110px_110px_50px] gap-4 items-center px-6 py-4 border-b border-lineBorder/70 dark:border-navy-100">
                  <Link href={`/products/${product.slug}`} className="hidden sm:flex items-center gap-4 min-w-0 group">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm font-semibold text-slateText dark:text-white line-clamp-1 group-hover:text-primary transition-colors">
                        {product.name}
                      </p>
                  ...
                  <span className="sm:text-center font-bold text-slateText dark:text-white text-sm sm:text-base">
                    {formatPrice(price)}
                  </span>

                  <div className="flex justify-start sm:justify-center">
                    <div className="flex items-center border border-lineBorder dark:border-navy-100 rounded-lg overflow-hidden">
                      <button
                        onClick={() => updateQuantity(product._id, quantity - 1)}
                        className="w-8 h-9 flex items-center justify-center text-muted hover:text-primary hover:bg-body dark:hover:bg-navy-100 transition-colors"
                        aria-label="Decrease"
                      >
                        <HiOutlineMinus className="w-3.5 h-3.5" />
                      </button>
                      <span className="w-9 text-center text-sm font-bold text-slateText dark:text-white">{quantity}</span>
                      <button
                        onClick={() => updateQuantity(product._id, quantity + 1)}
                        className="w-8 h-9 flex items-center justify-center text-muted hover:text-primary hover:bg-body dark:hover:bg-navy-100 transition-colors"
                        aria-label="Increase"
          ...
          <aside className="space-y-4 lg:sticky lg:top-28 h-fit">
              ...
              {isFreeShipping ? (
                ...
                <>
                  <p className="text-sm text-muted mb-2">
                    Add <span className="font-bold text-slateText dark:text-white">{formatPrice(FREE_SHIPPING - subtotal)}</span> more for
                    free shipping
                  </p>
                  <div className="h-2 bg-lineBorder dark:bg-navy-100 rounded-full overflow-hidden">
                    <div
            ...
            <div className="bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl p-6">
              <h2 className="text-lg font-bold text-slateText dark:text-white mb-5">Order Summary</h2>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted">Subtotal ({items.reduce((n, i) => n + i.quantity, 0)} items)</span>
                  <span className="font-semibold text-slateText dark:text-white">{formatPrice(subtotal)}</span>
                </div>
                <div className="flex justify-between">
                  ...
                  {shipping === 0 ? (
                    ...
                    <span className="font-semibold text-slateText dark:text-white">{formatPrice(shipping)}</span>
                  )}
                </div>
                <div className="flex justify-between">
                  <span className="text-muted">Tax</span>
                  <span className="font-semibold text-slateText dark:text-white">$0.00</span>
                </div>
              </div>
              <div className="border-t border-lineBorder dark:border-navy-100 my-4 pt-4 flex justify-between items-center">
      ...
      {items.length > 0 && (
        <div className="mt-16">
          <h2 className="text-2xl font-bold text-slateText dark:text-white mb-6">You Might Also Like</h2>
