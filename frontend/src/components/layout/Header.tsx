...
import { themeStore, applyTheme } from '@/store/themeStore';
    ...
    <header className={`sticky top-0 z-50 transition-transform duration-300 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                ...
                <div key={item.label} className="relative group">
                  ...
                  {hasDropdown && (
                    <div className="absolute left-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-1 group-hover:translate-y-0 z-50">
                      <div className="bg-white dark:bg-navy-200 rounded-xl shadow-nav border border-lineBorder dark:border-navy-100 py-2 w-60 overflow-hidden">
                        {item.label === 'Shop' ? (
                          <>
                            <Link
                              href="/shop"
                              className="block px-4 py-2.5 text-sm font-semibold text-navy dark:text-white hover:bg-primary/5 hover:text-primary transition-colors"
                            >
                              All Categories
                            </Link>
                            <div className="my-1 border-t border-lineBorder dark:border-navy-100" />
                            {categories.map((cat) => (
                              <Link
                                key={cat.slug}
                                href={`/shop?category=${cat.slug}`}
                                className="px-4 py-2 text-sm text-muted dark:text-white/80 flex items-center gap-2 hover:bg-primary/5 hover:text-primary transition-colors"
                              >
                          ...
                          brands.map((brand) => (
                            <Link
                              ...
                              className="px-4 py-2 text-sm text-muted dark:text-white/80 flex items-center gap-3 hover:bg-primary/5 hover:text-primary transition-colors"
                            >
      ...
      <AnimatePresence>
        {(openPanel === 'cart' || openPanel === 'wishlist' || openPanel === 'user' || openPanel === 'search') && (
          ...
          >
            <div className="bg-white dark:bg-navy-200 rounded-2xl shadow-nav border border-lineBorder dark:border-navy-100 overflow-hidden w-[360px] max-w-[calc(100vw-2rem)]">
              {openPanel === 'search' && (
                <div className="p-4">
                  <form onSubmit={handleSearch} className="relative">
                    <input
                      ...
                      className="w-full bg-body dark:bg-navy-100 border border-lineBorder dark:border-navy-100 rounded-lg pl-4 pr-11 py-2.5 text-sm outline-none focus:border-primary"
                    />
              ...
              {openPanel === 'user' && (
                <div className="py-2">
                  {authStatus === 'loading' ? (
                    ...
                    <>
                      <div className="flex items-center gap-3 px-4 py-3 border-b border-lineBorder dark:border-navy-100">
                        {user.avatar ? (
                          <img src={user.avatar} alt={user.username} className="w-10 h-10 rounded-full object-cover" />
                        ) : (
                          <div className="w-10 h-10 rounded-full bg-primary/10 text-primary dark:text-white flex items-center justify-center font-bold">
                            {user.username.charAt(0).toUpperCase()}
                          </div>
                        )}
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-slateText dark:text-white truncate">{user.username}</p>
                          <p className="text-xs text-muted truncate">{user.email}</p>
                        </div>
                      </div>
                      <Link href="/account" onClick={() => setOpenPanel(null)} className="flex items-center gap-2 px-4 py-3 text-sm text-slateText dark:text-white hover:bg-primary/5 hover:text-primary transition-colors">
                        <HiOutlineUser className="w-4 h-4" /> My Account
                      </Link>
                      <Link href="/wishlist" onClick={() => setOpenPanel(null)} className="flex items-center gap-2 px-4 py-3 text-sm text-slateText dark:text-white hover:bg-primary/5 hover:text-primary transition-colors">
                    ...
                    <>
                      <div className="px-4 py-3 border-b border-lineBorder dark:border-navy-100">
                        <p className="text-sm font-semibold text-slateText dark:text-white">Welcome to ElectroCart!</p>
                        <p className="text-xs text-muted">Login to manage your account.</p>
                      </div>
                      <Link href="/auth/login" onClick={() => setOpenPanel(null)} className="flex items-center gap-2 px-4 py-3 text-sm text-slateText dark:text-white hover:bg-primary/5 hover:text-primary transition-colors">
                        <HiOutlineUser className="w-4 h-4" /> Login
                      </Link>
                      <Link href="/auth/register" onClick={() => setOpenPanel(null)} className="flex items-center gap-2 px-4 py-3 text-sm text-slateText dark:text-white hover:bg-primary/5 hover:text-primary transition-colors">
              ...
              {openPanel === 'wishlist' && (
                <div>
                  <div className="flex items-center justify-between px-4 py-3 border-b border-lineBorder dark:border-navy-100">
                    <h4 className="font-semibold text-slateText dark:text-white text-sm">Wishlist ({wishlistItems.length})</h4>
                  ...
                  {wishlistItems.length === 0 ? (
                    ...
                    <div className="max-h-72 overflow-y-auto">
                      {wishlistItems.slice(0, 5).map((p) => (
                        <div key={p._id} className="flex items-center gap-3 px-4 py-3 border-b border-lineBorder/60 dark:border-navy-100">
                          <Link href={`/products/${p.slug}`} className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0">
                            <img src={p.image} alt={p.name} className="w-full h-full object-cover" />
                          </Link>
                          <div className="flex-1 min-w-0">
                            <Link href={`/products/${p.slug}`} className="text-xs font-medium text-slateText dark:text-white line-clamp-1 hover:text-primary">
                              {p.name}
                            </Link>
              ...
              {openPanel === 'cart' && (
                <div>
                  <div className="flex items-center justify-between px-4 py-3 border-b border-lineBorder dark:border-navy-100">
                    <h4 className="font-semibold text-slateText dark:text-white text-sm">Shopping Cart ({displayItemCount})</h4>
                    ...
                    {isFreeShipping ? (
                      ...
                      <div className="text-xs">
                        <p className="text-muted mb-1.5">
                          Add <span className="font-semibold text-navy dark:text-white">{formatPrice(FREE_SHIPPING - displaySubtotal)}</span> more for{" "}
                    ...
                    <>
                      <div className="max-h-64 overflow-y-auto">
                        {items.map(({ product, quantity }) => (
                          <div key={product._id} className="flex items-center gap-3 px-4 py-3 border-b border-lineBorder/60 dark:border-navy-100">
                            <Link href={`/products/${product.slug}`} className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0">
                              <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
                            </Link>
                            <div className="flex-1 min-w-0">
                              <Link href={`/products/${product.slug}`} className="text-xs font-medium text-slateText dark:text-white line-clamp-1 hover:text-primary">
                                {product.name}
                              </Link>
                              ...
                              <div className="flex items-center gap-1.5 mt-1.5">
                                <button
                                  ...
                                  className="w-6 h-6 border border-lineBorder dark:border-navy-100 rounded flex items-center justify-center text-muted hover:border-primary hover:text-primary transition-colors"
                                  aria-label="Decrease"
                                >
                                  <HiOutlineMinus className="w-3 h-3" />
                                </button>
                                <span className="text-xs font-semibold w-5 text-center text-slateText dark:text-white">{quantity}</span>
                                <button
                                  onClick={() => updateQuantity(product._id, quantity + 1)}
                                  className="w-6 h-6 border border-lineBorder dark:border-navy-100 rounded flex items-center justify-center text-muted hover:border-primary hover:text-primary transition-colors"
                                  aria-label="Increase"
                      ...
                      <div className="px-4 py-3 border-t border-lineBorder dark:border-navy-100">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-sm text-muted">Subtotal</span>
                          <span className="text-base font-bold text-slateText dark:text-white">{formatPrice(displaySubtotal)}</span>
                        </div>
                        <Link
                          href="/checkout"
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 w-80 max-w-[85vw] bg-white dark:bg-navy-200 shadow-2xl z-50 overflow-y-auto lg:hidden"
            >
              <div className="flex items-center justify-between p-4 border-b border-lineBorder dark:border-navy-100">
                <span className="text-xl font-bold text-navy dark:text-white">Electro<span className="text-accent">Cart</span></span>
                <button onClick={() => setMobileMenuOpen(false)} className="p-2 text-muted hover:text-navy dark:hover:text-white" aria-label="Close">
              ...
              <form onSubmit={handleSearch} className="p-4 pb-2 relative">
                <input
                  ...
                  className="w-full bg-body dark:bg-navy-100 border border-lineBorder dark:border-navy-100 rounded-lg pl-4 pr-10 py-2.5 text-sm outline-none focus:border-primary placeholder:text-muted"
                />
              ...
              <nav className="p-2">
                <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-sm font-medium text-slateText dark:text-white hover:bg-primary/5 hover:text-primary rounded-lg">
                  Home
                </Link>
                <Link href="/shop" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-sm font-medium text-slateText dark:text-white hover:bg-primary/5 hover:text-primary rounded-lg">
                  Shop
                </Link>
                <Link href="/deals" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-sm font-medium text-slateText dark:text-white hover:bg-primary/5 hover:text-primary rounded-lg">
                  Deals
                </Link>
                <Link href="/about" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-sm font-medium text-slateText dark:text-white hover:bg-primary/5 hover:text-primary rounded-lg">
                  About Us
                </Link>
                <Link href="/contact" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-sm font-medium text-slateText dark:text-white hover:bg-primary/5 hover:text-primary rounded-lg">
                  Contact Us
                </Link>

                <p className="px-4 pt-4 pb-2 text-xs font-bold text-muted uppercase tracking-wider">Categories</p>
                {categories.map((cat) => (
                  <Link key={cat.slug} href={`/shop?category=${cat.slug}`} onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-muted dark:text-white/80 hover:text-primary">
                    <cat.icon className="w-4 h-4" /> {cat.name}
                  </Link>
                ))}
                ...
                {brands.map((brand) => (
                  <Link key={brand._id} href={`/shop?brand=${encodeURIComponent(brand.name)}`} onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-muted dark:text-white/80 hover:text-primary">
                    <BrandLogo brand={brand} size={22} />
                    {brand.name}
                  </Link>
                ))}

                <div className="p-4 mt-2 border-t border-lineBorder dark:border-navy-100">
                  ...
                  <Link href="/auth/register" className="block w-full text-center border border-lineBorder dark:border-navy-100 text-slateText dark:text-white py-2.5 rounded-lg text-sm font-semibold">
                    Register
                  </Link>
                </div>
              </nav>
