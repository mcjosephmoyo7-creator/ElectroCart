      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-5">
        {brands.map((brand) => (
          <Link
            ...
            className="group flex items-center gap-4 bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl p-4 hover:border-accent hover:shadow-card-hover transition-all duration-300"
          >
            <span className="w-12 h-12 rounded-xl bg-body dark:bg-navy-100 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
              <BrandLogo brand={brand} size={34} />
            </span>
            <span className="font-semibold text-slateText dark:text-white text-sm truncate">{brand.name}</span>
          </Link>
        ))}
      </div>
