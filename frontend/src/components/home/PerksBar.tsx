...
export default function PerksBar() {
  return (
    <section className="bg-white dark:bg-navy-200 border-y border-lineBorder dark:border-navy-100">
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-lineBorder/70 dark:divide-navy-100">
          {perks.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="flex items-center gap-4 px-6 py-6 hover:bg-body dark:hover:bg-navy-100 transition-colors">
              <span className="w-12 h-12 shrink-0 bg-accent/10 rounded-xl flex items-center justify-center">
                <Icon className="w-6 h-6 text-accent" />
              </span>
              <div>
                <h4 className="font-semibold text-slateText dark:text-white">{title}</h4>
