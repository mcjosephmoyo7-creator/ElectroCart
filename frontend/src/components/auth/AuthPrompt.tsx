...
export default function AuthPrompt({ title, subtitle, benefits = ['wishlist', 'orders', 'secure'] }: AuthPromptProps) {
  ...
  return (
    <div className="max-w-md mx-auto bg-white dark:bg-navy-200 border border-lineBorder dark:border-navy-50 rounded-2xl p-8 shadow-card">
      <div className="mb-6 text-center">
        <div className="w-14 h-14 mx-auto rounded-full bg-primary/10 text-primary dark:text-white flex items-center justify-center mb-4">
          <FiLock className="w-6 h-6" />
        </div>
        <h1 className="text-xl font-bold text-slateText dark:text-white">{title}</h1>
        <p className="text-muted text-sm mt-1">{subtitle}</p>
      </div>

      <AuthForm mode={mode} onModeChange={setMode} redirectPath={pathname || '/'} onAuthenticated={() => undefined} />

      <div className="mt-6 pt-5 border-t border-lineBorder dark:border-navy-50 space-y-2.5">
        {benefits.map((b) => {
          const { Icon, label } = benefitMeta[b];
          return (
            <div key={b} className="flex items-start gap-2.5 text-sm text-muted">
              <Icon className="w-4 h-4 mt-0.5 shrink-0 text-primary dark:text-white" />
              <span>{label}</span>
            </div>
          );
        })}
