...
export default function PriceRangeSlider({ min, max, value, onChange }: PriceRangeSliderProps) {
  ...
  <label className="flex-1 flex items-center gap-1.5 bg-body  border border-lineBorder  rounded-lg px-2 py-1.5">
  ...
  value={value[0]}
  onChange={(e) => setLower(e.target.value)}
  className="w-full bg-transparent text-slateText  text-sm outline-none"
  />
  </label>
